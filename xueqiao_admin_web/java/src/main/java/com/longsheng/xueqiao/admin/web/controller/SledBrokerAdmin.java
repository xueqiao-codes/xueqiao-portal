...
public class SledBrokerAdmin extends AbstractDBLockController {
	...
	public void show() throws Exception {
		...
		// AppLog.i("SledExchangeAdmin ---- show ---- requestPage : " +
		// requestPage);
		if (requestPage < 1) {
			requestPage = 1;
		}
...
//		} else {
			requestPage = queryBrokerEntryList(requestPage - 1, brokerId, engName, cnName, brokerPlatform);
//		}
		...
		put("engName", engName);
		put("cnName", cnName);
		put("brokerPlatform", brokerPlatform);
	...
	private int queryBrokerEntryList(int pageIndex, int brokerId, String engName, String cnName, String brokerPlatform) throws TException {
		...
		if (brokerPlatform != null && !brokerPlatform.isEmpty()) {
			option.setPlatform(BrokerPlatform.valueOf(brokerPlatform));
		}
		...
		mTotalNum = brokerEntryPage.getTotal();
	...
	private int queryBrokerEntryListByPlatform(int pageIndex, String platform) throws TException {
		...
		mTotalNum = brokerAccessEntryPage.getTotal();
