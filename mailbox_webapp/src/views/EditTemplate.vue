...
export default {
  ...
  methods: {
    ...
    updateTemplate() {
      ...
      template.templateId = this.selectedTemplate.templateId;
      ...
      this.$api.updateTemplate(template, function(response) {
        if (response != null) {
          if (response.success) {
            alert("模板修改成功");
