...
public class ContractServiceStub extends BaseStub {
  ...
  public SyncCommodityConfigResp  syncSledCommodityConfig(int sledCommodityId) throws org.soldier.platform.svr_platform.comm.ErrorInfo, TException{
    return syncSledCommodityConfig(sledCommodityId, new TStubOption().setCallTrace(Thread.currentThread().getStackTrace()[2]));
  }

  public SyncCommodityConfigResp  syncSledCommodityConfig(int sledCommodityId,TStubOption platformStubOption) throws org.soldier.platform.svr_platform.comm.ErrorInfo, TException {
    InvokeMethodInfo invokeInfo = new InvokeMethodInfo("syncSledCommodityConfig").setRouteKey(platformStubOption.getRouteKey()).setTimeoutMs(platformStubOption.getTimeoutMs());
    if (platformStubOption.getCallTrace() != null) { 
      invokeInfo.setSourceCallTrace(platformStubOption.getCallTrace());
    } else {
      invokeInfo.setSourceCallTrace(Thread.currentThread().getStackTrace()[2]);
    }
    return super.runSync(new ThriftCallable<SyncCommodityConfigResp>(){
    @Override
    public SyncCommodityConfigResp call(TProtocol protocol, PlatformArgs platformArgs) throws ErrorInfo, TException {
      return new ContractService.Client(protocol).syncSledCommodityConfig(platformArgs, sledCommodityId);
      }
    }, invokeInfo);
  }

  public SyncCommodityConfigResp  syncSledCommodityConfig(int routeKey, int timeout,int sledCommodityId)throws org.soldier.platform.svr_platform.comm.ErrorInfo, TException{
    return syncSledCommodityConfig(sledCommodityId, new TStubOption().setRouteKey(routeKey).setTimeoutMs(timeout).setCallTrace(Thread.currentThread().getStackTrace()[2]));
  }
