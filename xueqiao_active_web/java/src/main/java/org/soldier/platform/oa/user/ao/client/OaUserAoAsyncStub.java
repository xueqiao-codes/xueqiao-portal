...
public class OaUserAoAsyncStub { 
  ...
  public void send_logout(int routeKey, int timeout, int userId) throws TException {
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":" 
    ...
    SvrContainer.getInstance().sendRequest(
        create_logoutServiceCall(routeKey, timeout, platformArgs, userId), new TRequestOption());
  }

  public void send_logout(int routeKey, int timeout, int userId,TRequestOption requestOption) throws TException { 
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":" 
    ...
    SvrContainer.getInstance().sendRequest(
        create_logoutServiceCall(routeKey, timeout, platformArgs, userId), requestOption);
  }

  public long logout(int routeKey, int timeout, int userId, IMethodCallback<OaUserAo.logout_args, OaUserAo.logout_result> callback) throws TException{
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":"
    ...
    return SvrContainer.getInstance().sendRequest(
            create_logoutServiceCall(routeKey, timeout, platformArgs, userId), callback);
  }

  public long add_logoutCall(AsyncCallRunner runner, int routeKey, int timeout, int userId, IMethodCallback<OaUserAo.logout_args, OaUserAo.logout_result> callback) throws TException{
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":" 
  ...
  protected TServiceCall create_logoutServiceCall(int routeKey, int timeout, org.soldier.platform.svr_platform.comm.PlatformArgs platformArgs, int userId){
    TServiceCall serviceCall = new TServiceCall();
    serviceCall.setServiceKey(OaUserAoVariable.serviceKey);
    serviceCall.setRouteKey(routeKey);
    serviceCall.setOneWay(false);
