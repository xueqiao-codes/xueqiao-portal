...
public class CommodityRegisterHandler {
    ...
    public CommodityRegisterPage reqCommodityRegister(ReqCommodityRegisterOption option,
        ...
        if (page.getPageSize() > 0) {
            ...
            for (ContractRegisterRule rule : page.getPage()) {
                ...
                register.setMaxActiveCount(rule.getMaxActiveCount());
                register.setMaxInactiveCount(rule.getMaxInactiveCount());
    ...
    private ContractRegisterRule map2ContractRegisterRule(CommodityRegister commodityRegister) {
        ...
        if (commodityRegister.isSetMaxActiveCount()) {
            registerRule.setMaxActiveCount(commodityRegister.getMaxActiveCount());
        }
        if (commodityRegister.isSetMaxInactiveCount()) {
            registerRule.setMaxInactiveCount(commodityRegister.getMaxInactiveCount());
        }
        return registerRule;
    }

    public void deleteCommodityRegister(int sledCommodityId, TechPlatformEnv platformEnv) throws TException {
        IQuotationAccountApi iQuotationAccountApi = QuotationAccountApiFactory.getInstance().getDefault();
        iQuotationAccountApi.deleteContractRegisterRule(sledCommodityId, BeanMapping.map2QuoPlatformEnv(platformEnv));
    }
